...
~~~
注意力通常可以解决梯度消失问题, 因为其在encoder status与decoder之间建立了直接连接。与CNN中的跳丽连接非常相似。
~~~
...
综上可以发现, 将多头的输出变为$(B, H, N, d_h)$是为了方便先进行注意力分数计算, 等拼接时再去reshape。
...
**Positional Encoding** 位置编码为每个嵌入向量添加位置编码。 因为Transformer不像RNN和LSTM等模型一样有隐向量和递归结构。递归本身就隐含了位置/顺序关系。
