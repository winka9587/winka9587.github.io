...
首先写入三维点的数量len(img.point3D_ids), 对应读取代码中**num_points2D**, 可以发现这里存储的值是**三维点云在当前相机视角画面上对应的2D点的数量**。

使用zip将xys和p3d_id这两个长度相同的list打包为一对一元组, ([x, y], p3d_id)循环写入.bin文件中。同样对应读取不分的代码来理解：
