...
协方差这一部分可以去看另一篇: [**[Gaussian Splatting] 协方差矩阵与分解**](/_posts/2024-04-10-3dg_cov3d.md)



## 使用RGBD序列初始化3dgs

**需要注意的是, colmap的结果都是W2C的**
...
能否学习PoseFree那篇论文, 将深度点放到同一空间下, 进行低通处理？（这要求pose作为先验）

### 假设

我们有n张图像$I_i$, 只有其中第j张图像的深度图$D_j$, 因为我们无法获得相机之间的关系, 帧与帧之间的相对位姿是不可知的。

使用第j帧的RGBD来进行初始化, 我们得到一个带颜色的点云$P_j$, 然后用其他帧来约束？How？
每个点初始化为球体, 投影（问题在于, 投影需要位姿）到各个画面上计算loss, 在连续的画面上应该容易对齐。

能否利用光线追踪的思路, 其他帧使用RGB来得到一个点云？

这里需要的功能:

1. 输入RGBD, 输出带颜色的点云, 并使用该点云初始化一个3dgs
2. 将这个3dgs投影到其他视角的图像上, 但是这里需要一个位姿才能进行投影

以前的24Sig使用ICP进行初始匹配, 然后使用深度图来做loss, 进一步对齐, 那我这里是否同样可以用深度图？

但是其实到这一步为止, 都不需要3dgs, 用点云就能完成。

**做动态物体的3dgs, 现在的方法都要求场景静止, 或者说相机围绕一个物体运动. 一旦物体动起来之后, 就将失败**

想达到的效果:

**给定初始mask+位姿, 拿着手机拍一段物体运动的视频, 得到重建出来的物体的3dgs（必须划分与4dgs的不同, 针对单个物体而非整个场景, 3dgs部分能够辅助位姿估计）（而且在未来, 3dgs的编辑中, 是否会需要物理碰撞？单个物体放到一个新的场景中，我们或许并不希望它仅仅只是能摆放在那里）（未来的后续动作, 单个物体跟踪, 但是这个物体的部件是可动的, 例如遥控汽车的轮子, 笔记本开关, 布偶熊的手臂）**

**实验部分, 在RGBD数据集上测试, 测试不同帧的深度图进行初始化的结果差异。在4Dgs的数据集上测试, 使用深度估计来得到初始结果**

之前测试过, 使用RGB+mask输入到colmap中, 结果失败

如果换成跟踪的pose + RGB + mask, 能否在gs中重建这个物体?


**还需要考虑一个问题: 即便是同一个surface点, 在不同视角下, 因为光线等问题, 也可能会有不同的颜色**

### 假设 [现在的图像就是3dgs渲染出来的]

1. ICP获得一个初值是必须的, 但是未必要依赖于这个初值


### 测试1, 现有数据集的pose代替colmap

1. 点云从哪来?

从模型得到, 但是还需要每帧图像的2D点映射关系
